...
def processing_user_files(chat_id, user_name):
    ...
    pdf_files_list = get_all_user_files(chat_id, user_name)
...
def separate_file(chat_id, user_name, file_path):
...
def get_vectror_db(chat_id, user_name):
    ...
    vectordb = Chroma(persist_directory=db_folder, embedding_function=model_llm)

    return vectordb

def get_answer(chat_id, user_name, vectordb, prompt):
